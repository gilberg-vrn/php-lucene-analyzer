 ...
 */
class ByteBuffersDataOutput extends DataOutput
{
    private $EMPTY = [];
    private $EMPTY_BYTE_ARRAY = [];

    const ALLOCATE_BB_ON_HEAP = 0;
    private $blocks = [];
    ...
    public function writeBytes($b, $offset, $length = null)
    {
        ...
        if (is_string($b)) {
            if (strlen($b) === 1) {
                ...
                foreach (preg_split('//u', $b, -1, PREG_SPLIT_NO_EMPTY) as $char) {
                    if ($skip++ < $offset) {
                        continue;
                    }
                    if ($skip - $offset > $length) {
                        break;
                    }
                    $this->writeByte($char);
                }
    ...
    public function toArrayCopy()
    {
        ...
        $arr = []; //new byte[Math.toIntExact(size())];
        $offset = 0;
        foreach ($this->toBufferList() as $bb) {
            $len = $bb->remaining();
    ...
    public function size()
    {
        ...
        if ($blockCount >= 1) {
            $fullBlockSize = ($blockCount - 1) * $this->blockSize();
            $lastBlockSize = $this->blocks->getLast()->position();
