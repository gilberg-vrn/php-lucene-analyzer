 ...
 */
final class EnchantStemFilter extends TokenStream
{
  private $stemmer;
   ...
   * @param string $dictionary HunspellDictionary containing the affix rules and words that will be used to stem the tokens
   * @param bool $dedup
   * @param bool $longestOnly true if only the longest term should be output.
   */
public function __construct(TokenStream $input, $dictionary, bool $dedup = true, bool $longestOnly = false) {
    parent::__construct($input);
    $this->dedup = $dedup && $longestOnly == false; // don't waste time deduping if longestOnly is set
    $this->stemmer = new EnchantStemmer($dictionary);
    ...
    public function incrementToken(): bool
    {
        ...
        $this->buffer = $this->stemmer->uniqueStems($this->termAttribute, mb_strlen($this->termAttribute));
//        $this->buffer = $this->dedup ? $this->stemmer->uniqueStems($this->termAttribute, mb_strlen($this->termAttribute)) : $this->stemmer->stemWord($this->termAttribute);
