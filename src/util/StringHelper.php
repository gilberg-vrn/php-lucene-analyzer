 ...
 * Class StringHelper
 *
 * @package ftIndex\util
 * @author  Dmitrii Emelyanov <gilberg.vrn@gmail.com>
 * @date    9/25/19 2:04 PM
 */
...
//
   ...
   * @param BytesRef $prefix
   *          the expected prefix
   * @return bool Returns <code>true</code> iff the ref starts with the given prefix.
   *         Otherwise <code>false</code>.
   */
    public static function startsWith(BytesRef $ref, BytesRef $prefix): bool
    {
        return self::sliceEquals($ref, $prefix, 0);
    }
//
//  /**
...
//}
//
    private static function sliceEquals(BytesRef $sliceToTest, BytesRef $other, int $pos): bool
    {
        if ($pos < 0 || $sliceToTest->length - $pos < $other->length) {
            return false;
        }
        $i = $sliceToTest->offset + $pos;
        $j = $other->offset;
        $k = $other->offset + $other->length;
